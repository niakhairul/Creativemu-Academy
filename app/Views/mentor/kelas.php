...
<?php endif; ?>
<script>
document.querySelectorAll('.kartu-kelas').forEach(function (kartu) {
    const aksi = kartu.querySelector('.d-grid');
    const lihat = aksi?.querySelector('a[href*="/mentor/kelas/"]');
    if (!aksi || !lihat) return;
    const tombol = document.createElement('a');
    tombol.href = lihat.href.replace(/\/?$/, '/ujian-tugas');
    tombol.className = 'btn btn-outline-success w-100 mt-2';
    tombol.innerHTML = '<i class="fa-solid fa-clipboard-check me-1"></i> Tambah Ujian & Tugas';
